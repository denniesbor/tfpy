...
def run_all_models():
    ...
    for site in selected_sites:
        ...
        try:
            ...
            logger.info(
                f"  Model 3 (TD):     PE={m3_td['pe']:.4f}  ({m3_td_time:.1f}s)"
            )
            logger.info(
                f"  Model 3 (FD):     PE={m3_fd['pe']:.4f}  ({m3_fd_time:.1f}s)"
            )
            logger.info(f"  Model 4:          PE={m4['pe']:.4f}  ({m4_time:.1f}s)")
            logger.info(
                f"  Heyns TD:         PE={heyns_td_result['pe']:.4f}  ({heyns_td_time:.1f}s, n={heyns_td_result['ensemble_size']})"
            )
            logger.info(
                f"  Heyns Ensemble:   PE={heyns_ens_result['pe']:.4f}  ({heyns_ens_time:.1f}s, n_win={heyns_ens_result['n_windows']})"
            )
...
def save_results(results):
    ...
    for site, site_results in results.items():
        ...
        for model_name, model_result in site_results.items():
            json_entry = {
                ...
                "predictions": (
                    model_result.get(
                        "pred", model_result.get("predictions", [])
                    ).tolist()
                    if hasattr(
                        model_result.get("pred", model_result.get("predictions", [])),
                        "tolist",
                    )
                    else list(
                        model_result.get("pred", model_result.get("predictions", []))
                ...
                "observations": (
                    model_result.get(
                        "measured", model_result.get("observations", [])
                    ).tolist()
                    if hasattr(
                        model_result.get(
                            "measured", model_result.get("observations", [])
            ...
            if "pred_lower" in model_result and "pred_upper" in model_result:
                ...
                json_entry["confidence_bounds"] = {
                    "lower": (
                        pred_lower.tolist()
                        if hasattr(pred_lower, "tolist")
                        else list(pred_lower)
                    ),
                    "upper": (
                        pred_upper.tolist()
                        if hasattr(pred_upper, "tolist")
                        else list(pred_upper)
                    ),
                }
                ...
                if "training_history" in model_result:
                    ...
                    if isinstance(history, dict) and "loss" in history:
                        ...
                        if "val_loss" in history:
                            json_entry["final_validation_loss"] = float(
                                history["val_loss"][-1]
                            )
    ...
    logger.info(
        "Results saved with confidence bounds - ready for exploration in Jupyter notebook!"
    )
...
def main():
    ...
    logger.info(
        f"Total execution time: {elapsed_time:.1f} seconds ({elapsed_time/60:.1f} minutes)"
    )
