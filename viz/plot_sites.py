...
def plot_sites(selected_sites, tva_gic, tva_mag, closest_mag_to_gic, tl_gdf):
    ...
    legend_elements = [
        plt.scatter(
            [], [], color="red", marker="s", s=120, edgecolor="k", label="GIC Monitor"
        ),
        plt.scatter(
            [], [], color="blue", marker="o", s=100, edgecolor="k", label="Magnetometer"
        ),
        plt.scatter(
            [], [], color="green", marker="^", s=100, edgecolor="k", label="MT Station"
        ),
        *[
            plt.plot([], [], color=voltage_colors[v], linewidth=2, label=f"{v} kV")[0]
            for v in voltages
        ],
    ]
