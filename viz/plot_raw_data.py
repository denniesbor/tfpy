...
def plot_single_site(
    axes, time_axis, Bx, By, Ex, Ey, gic_data, site, mt_site, closest_mag_to_gic
):
    ...
    mt_dist = haversine_dist(
        info["lat"], info["lon"], mt_site.latitude, mt_site.longitude
    )

    ax1.text(
        0.02,
        0.85,
        f"Mag: {mag_dist:.1f} km to {info['magnetometer']}",
    ...
    ax2.text(
        0.02,
        0.85,
        f"MT: {mt_site.name} {mt_dist:.1f} km",
        transform=ax2.transAxes,
        fontsize=8,
    )
    ax3.text(0.02, 0.85, f"GIC: {site}", transform=ax3.transAxes, fontsize=8)
    ax2.text(
        1.02, 0.5, site, transform=ax2.transAxes, rotation=90, va="center", fontsize=10
    )
