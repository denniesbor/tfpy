...
from viz.plot_utils import setup_matplotlib, save_figure, find_common_time_window, align_model_results
...
def _plot_single_site(fig, gs_row, site, full_results, tva_gic,
                      common_start, common_end, bottom=False):
    ...
    for i, (name, color) in enumerate(MODEL_COLORS.items()):
        ...
        ax.text(1.07, 0.5, f"{label} (PE: {d['pe']:.2f})",
                transform=ax.transAxes, fontsize=9, color="k", va="center")
...
def plot_all_sites(selected_sites, tva_gic, closest_mag_to_gic):
    ...
    for i, site in enumerate(selected_sites):
        if site not in full_results:
            ...
            continue
        _plot_single_site(
            fig, i, site, full_results, tva_gic,
