...
class ImpedanceLayer(nn.Module):
    ...
    def forward(self, Bx_f, By_f):
        """
        Forward pass for impedance layer.
        """
        Z = self.Z0 + self.dZ
...
class GICPINN(nn.Module):
    ...
    def __init__(self, Z0, n_other=15, hidden=64):
        ...
        # Main GRU with increased capacity
        gru_input_size = 2 + 2 + n_other - 2  # Bx,By, Ex,Ey, other(13)
        self.gru = nn.GRU(
    ...
    def forward(self, x):
        """
        Forward pass for GICPINN.
        """
        Bx, By = x[:, 0], x[:, 1]
        other = x[:, 2:]  # (B,C-2,win)

        # Impedance layer processing
        ...
        # Attention mechanism - let model focus on important time steps
        attended_out, attn_weights = self.attention(gru_out, gru_out, gru_out)
        
        # Use attention-weighted final state
        final_state = torch.cat([h[-2], h[-1]], 1)  # (B, 2*hidden)
        attended_final = (attn_weights.mean(1).unsqueeze(-1) * attended_out).sum(1)  # True attention pooling
        
        # Combine GRU final state with attended features
...
def train_pinn(model, dl_tr, dl_val, epochs=40, lr=3e-4, yard="", β_E=0.2, λ_Z=1e-3, device=None):
    """
    ...
    best, patience, best_state = 1e9, 8, None  # Increased patience
    ...
    for ep in range(epochs):
        ...
        for step, (xb, eref, yb) in enumerate(tqdm(dl_tr, desc=f"train {yard} ep{ep}", leave=False)):
            ...
            # Add smoothness regularization for impedance
            L_smooth = torch.mean(torch.abs(model.imp.dZ[1:] - model.imp.dZ[:-1])) if model.imp.dZ.shape[1] > 1 else 0
            ...
            opt.zero_grad()
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 0.5)  # Reduced clipping
            opt.step()
        ...
        with torch.no_grad():
            for xb, eref, yb in dl_val:
                ...
                val_loss += (F.mse_loss(p, yb) + 
                            β_E * F.mse_loss(torch.stack([Ex, Ey], 1), eref)).item()
                            
        val_loss /= len(dl_val)
        scheduler.step(val_loss)  # Learning rate scheduling
        
        logger.info(f"{yard} ep{ep:02d} | train {train_loss/len(dl_tr):.5f} | val {val_loss:.5f} | lr {opt.param_groups[0]['lr']:.2e}")

        if val_loss < best - 1e-5:  # Reduced threshold
...
def leave_one_yard_out(yards, build_feature_tensor_fn, win=256, batch=1024, epochs=40, device=None):
    """
...
def train_on_all_yards(yards, build_feature_tensor_fn, win=256, batch=1024, epochs=40, device=None):
    """
    ...
    # model
    model = GICPINN(dummy_Z, n_other=15, hidden=64).to(device)
    trained_model = train_pinn(model, dl_tr, dl_val, epochs=epochs, lr=3e-4, yard="all_yards", device=device)
    
    return trained_model


def train_temporal_split(yard, build_feature_tensor_fn, win=256, batch=1024, epochs=40, device=None):
    ...
    # Split: Days 1-2 train, Day 3 test (assuming 86400 samples/day)
    samples_per_day = len(y_full) // 3  # 3 days total
    
    # Training data (Days 1-2)
    ...
    test_ds = WindowDS(X_test, y_test, E_test, win, stats=train_ds.stats, y_stats=train_ds.y_stats)
    ...
    dummy_Z = np.zeros((4, winF), dtype=np.complex64)
    model = GICPINN(dummy_Z, n_other=15, hidden=64).to(device)
    trained_model = train_pinn(model, dl_tr, dl_val, epochs=epochs, lr=3e-4, yard=f"{yard}_temporal", device=device)
