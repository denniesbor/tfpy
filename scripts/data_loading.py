"""
Author: Dennies Bor
Role:   Data loading functions for GIC analysis.
"""
...
def load_mt_sites(filename=None):
    ...
    if not filename.exists():
        logger.error(f"MT sites pickle file {filename} not found.")
        raise FileNotFoundError(f"MT sites pickle file {filename} not found.")
...
def load_transmission_lines(filename=None):
    """Load pre-filtered transmission lines from pickle file."""
    if filename is None:
        filename = get_data_dir() / "trans_lines_within_FERC_filtered.pkl"

    if not filename.exists():
        logger.error(f"Transmission lines pickle file {filename} not found.")
        raise FileNotFoundError(f"Transmission lines pickle file {filename} not found.")
...
def find_closest_mt_sites_to_magnetometers(mt_sites=None, mag_data=None):
    """Find the closest MT site for each magnetometer device."""
    if mt_sites is None:
...
def find_closest_magnetometers_to_mt_sites(mt_sites=None, mag_data=None):
...
def find_closest_magnetometers_to_gic(
    ...
    """
    For every GIC monitoring device return nearest magnetometer, primary MT site,
    and all MT sites within radius_km of the GIC yard.
    """
    if gic_data is None:
    ...
    for device in gic_data.device.values:
        ...
        logger.info(
            f"GIC {device}: nearest magnetometer {closest_mag} ({min_dist:.2f} km), "
            f"MT sites within {radius_km} km: {', '.join(site.name for site in mt_cluster)}."
        )
