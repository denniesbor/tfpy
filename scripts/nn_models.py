...
def feature_matrix_cnn(site, mag_data, gic_data, site_relations, start, end):
    ...
    bx = mag_data.sel(
        device=site_relations[site]["magnetometer"], time=slice(start, end)
    ).Bx.values.astype("f4")
    by = mag_data.sel(
        device=site_relations[site]["magnetometer"], time=slice(start, end)
    ).By.values.astype("f4")
...
def model5b_gru(
    ...
    ds_val, ds_train = torch.utils.data.random_split(
        ds_tr, [n_val, len(ds_tr) - n_val], generator=torch.Generator().manual_seed(42)
    )
    ...
    for i, (model_class, hidden) in enumerate(ensemble_configs):
        ...
        val_loss = train_model(
            model, dl_tr, dl_val, epochs=epochs, model_name=f"GRU-{i}"
        )
