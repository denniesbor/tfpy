...
def model3_td(
    ...
    for i in range(n_bootstrap):
        ...
        try:
            a_boot, b_boot = np.linalg.lstsq(
                np.column_stack([Ex_boot, Ey_boot]), gic_boot, rcond=None
            )[0]
...
def model3_fd(
    ...
    for i in range(n_bootstrap):
        ...
        g_boot, bx_boot, by_boot = (
            taper(g_boot, w_boot),
            taper(bx_boot, w_boot),
            taper(by_boot, w_boot),
        )

        try:
            freqs_boot, Ex_f_boot, Ey_f_boot = mt.convolve_fft_freq(
                bx_boot, by_boot, dt=1.0
            )
    ...
    bootstrap_preds = []
    for tf_boot in bootstrap_tfs[:20]:
...
def heyns_td(
    ...
    logger.info(
        f"[{site}] Starting Heyns TD with temporal subsampling (every {subsample_step}s)..."
    )
    ...
    logger.info(
        f"[{site}] Heyns TD complete: PE_median = {pe_median:.4f}, PE_mean = {pe_mean:.4f}"
    )
...
def heyns_ensemble(
    ...
    logger.info(
        f"[{site}] Window: {window_pts} pts ({window_pts*dt_seconds/3600:.1f}h), Shift: {shift_pts} pts"
    )
