...
def _load_train_test(site, gic_data, mag_data, site_relations, train_slice, test_slice, taper_pct):
    ...
    return (
        taper(g_tr[:n], w), taper(bx_tr[:n], w), taper(by_tr[:n], w), n,
        g_ts[:m], taper(bx_ts[:m], w_ts), taper(by_ts[:m], w_ts), m,
    )
...
def model3_td(
    site, gic_data, mag_data, site_relations,
    train_slice, test_slice, pad_factor=1.0, taper_pct=0.05, n_bootstrap=100,
):
...
def model3_fd(
    site, gic_data, mag_data, site_relations,
    train_slice, test_slice, n_eval=28, taper_pct=0.05, n_bootstrap=50,
):
    ...
    N2     = 2 * (len(freqs2) - 1)
    g_pred = edge_blend(np.real(np.fft.irfft(_apply_tf(tf, freqs2, Ex_f2, Ey_f2), n=N2))[:m])

    boot_preds = []
    for tf_b in boot_tfs[:20]:
        boot_preds.append(
            edge_blend(np.real(np.fft.irfft(_apply_tf(tf_b, freqs2, Ex_f2, Ey_f2), n=N2))[:m])
        )
...
def model4_fd(
    site, gic_data, mag_data, site_relations,
    train_slice, test_slice, n_eval=28, pad_factor=1.5,
    taper_pct=0.05, ridge=1e-4, n_bootstrap=50,
):
    ...
    def _fit_b_bands(bx_f, by_f, gic_f, freqs):
        ...
        for f_lo, f_hi in bands:
            ...
            coef = robust_complex_lsq(A, np.hstack([g_b.real, g_b.imag]), δ=1.345, max_iter=20, tol=1e-6)
    ...
    for _ in range(n_bootstrap):
        ...
        try:
            ...
            boot_tfs.append(_fit_b_bands(np.fft.rfft(bx_p), np.fft.rfft(by_p), np.fft.rfft(gi_p), fr_b))
        except Exception:
            continue
    ...
    pred = edge_blend(np.real(np.fft.irfft(_apply_tf(tf, freqs_ts, bx_f_ts, by_f_ts)))[p_ts: p_ts + m])

    boot_preds = [
        edge_blend(np.real(np.fft.irfft(_apply_tf(tb, freqs_ts, bx_f_ts, by_f_ts)))[p_ts: p_ts + m])
...
def heyns_td(
    site, gic_data, mag_data, site_relations,
    train_slice, test_slice, taper_pct=0.05,
    min_gic_thresh=0.1, subsample_step=60, max_pairs=None,
):
...
def heyns_ensemble(
    site, gic_data, mag_data, site_relations,
    train_slice, test_slice, window_hours=8, shift_hours=0.5,
    n_eval=28, taper_pct=0.05, pad_factor=1.5,
):
    ...
    logger.info(f"{site}: {len(starts)} windows, win={win_pts} pts, shift={shift_pts} pts.")
    ...
    pred = edge_blend(
        np.real(np.fft.irfft(_apply_tf(tf_median, freqs_ts, bx_f_ts, by_f_ts)))[p_ts: p_ts + m]
    )
    ...
    for (f_lo, f_hi), ests in tf_ensemble.items():
        ...
        for idx in np.random.choice(len(ests), min(20, len(ests)), replace=False):
            ...
            boot_preds.append(
                edge_blend(np.real(np.fft.irfft(g_f_s))[p_ts: p_ts + m])
            )
