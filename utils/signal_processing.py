...
def fix_nans(arr, max_gap_pts=600):
    ...
    s = pd.Series(arr)
    arr[:] = (
        s.ffill(limit=max_gap_pts).bfill(limit=max_gap_pts).to_numpy(dtype=arr.dtype)
    )
...
def clean_timeseries(ds, var, max_gap=60):
    ...
    limit_points = max_gap
    da_filled = da_interp.ffill("time", limit=limit_points).bfill(
        "time", limit=limit_points
    )
...
def patch_bezpy_tl():
    ...
    def calc_voltage_components(self, E, how="delaunay"):
        """
        Return tuple (Vx, Vy) in volts, keeping the Ex·dx and Ey·dy pieces separate.
