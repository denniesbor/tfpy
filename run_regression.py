    ...
    for site in sites:
        ...
        models = [
            (
                "3_td",
                lambda: model3_td(
                    site, tva_gic, tva_mag, site_rel, TRAIN_SLICE, TEST_SLICE
                ),
            ),
            (
                "3_fd",
            ...
            (
                "5a",
                lambda: model5a_cnn(
                    site, tva_mag, tva_gic, site_rel, TRAIN_SLICE, TEST_SLICE, epochs=40
                ),
            ),
            (
                "5b",
...
def _build_json_entry(model_name, res):
    ...
    if model_name == "heyns_td":
        ...
        if "coeffs" in res and len(res["coeffs"]) == 2:
            entry["coefficients"] = {
                "alpha": float(res["coeffs"][0]),
                "beta": float(res["coeffs"][1]),
            }
        if "transfer" in res:
...
def save_results(results):
    ...
    avg_row = {
        "Site": "Average",
        **{c: df[c].mean() for c in df.columns if c != "Site"},
    }
